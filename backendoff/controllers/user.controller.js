...
exports.getStudyTime = async (req, res) => {
  try {
    ...
    const studyTimeData = await userService.getStudyTime(studentId, sevenDaysAgo);
    ...
    studyTimeData.forEach(record => {
      const dayIndex = record.date.getDay();
      formattedData[dayIndex].hours += record.duration / 60; // Convert minutes to hours
    });

    res.json(formattedData);
  } catch (error) {
    console.error('Error fetching study time:', error);
    res.status(500).json({ message: 'Error fetching study time data' });
  }
};
...
exports.getProgress = async (req, res) => {
  try {
    const studentId = req.params.id;
    const progressData = await userService.getStudentProgress(studentId);
...
exports.getEnrolledCourses = async (req, res) => {
  try {
    const studentId = req.params.id;
    const courses = await userService.getEnrolledCourses(studentId);
